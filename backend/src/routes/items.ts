...
function stockFromBody(body: {
  ...
  const stockG =
    parseFloat(
      String(body.currentNet ?? body.currentQty ?? '0')
    ) || 0;

  const purity =
    parseFloat(String(body.purity ?? '100')) || 100;

  const fineG =
    body.currentFine != null &&
    String(body.currentFine) !== ''
    ...
    try {
      ...
      const [created] = await db
        ...
        .values({
          ...
          saleRate: toStr(
            body.saleRate ?? body.purchaseRate,
            '0'
          ),
          purchaseRate: toStr(
            body.purchaseRate,
  ...
  async (req, res, next) => {
    try {
      ...
      if (body.huid !== undefined) {
        patch.huid = body.huid;
      }

      if (body.metalType != null) {
        patch.metalType = body.metalType;
      }

      if (body.purity != null) {
        patch.purity = toStr(body.purity);
      }

      if (body.finePercent != null) {
        patch.finePercent = toStr(
          body.finePercent
        );
      }

      if (body.wastage != null) {
        patch.wastage = toStr(body.wastage);
      }

      if (body.makingCharge != null) {
        patch.makingCharge = toStr(
          body.makingCharge
        );
      }

      if (body.labourCharge != null) {
        patch.labourCharge = toStr(
          body.labourCharge
        );
      }

      if (body.otherCharge != null) {
        patch.otherCharge = toStr(
          body.otherCharge
        );
      }

      if (body.saleRate != null) {
        patch.saleRate = toStr(
          body.saleRate
        );
      }

      if (body.purchaseRate != null) {
        patch.purchaseRate = toStr(
          body.purchaseRate
        );
      }

      if (body.minStock != null) {
        patch.minStock = toStr(
          body.minStock
        );
      }
      ...
      if (
        body.currentNet != null ||
        body.currentQty != null ||
        body.currentFine != null
      ) {

          ...
          async (tx) => {
            ...
            for (
              ...
              await tx
                ...
                .set({
                  currentQty:
                    sql`${items.currentQty} - ${String(line.quantity || 1)}`,
                  currentGross:
                    sql`${items.currentGross} - ${String(line.grossWeight)}`,
                  currentNet:
                    sql`${items.currentNet} - ${String(line.netWeight)}`,
                  currentFine:
                    sql`${items.currentFine} - ${String(line.fineWeight || 0)}`,
                  updatedAt:
                    new Date(),
                })
                .where(
            ...
            if (
              ...
              await tx
                ...
                .values({
                  ...
                  narration:
                    `Received Fine against ${invoiceNo}`,
                  createdBy:
                    req.user!.id,
                });
            }
              ...
              await tx
                ...
                .values({
                  ...
                  fineCredit:
                    '0',
                  narration:
                    `Rate Cut ${body.rateCut?.fine}g @ ${body.rateCut?.rate} - ${invoiceNo}`,
                  createdBy:
                    req.user!.id,
                });
            }
